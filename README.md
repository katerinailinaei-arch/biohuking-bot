...
В том же процессе можно нажать «Темы». Темы берутся так: вы пересылаете боту пост из канала-ориентира (список: /sources). Бот помогает переписать идею своими словами; чужие ролики и дословный текст в канал не копирует. Компьютер должен быть включён и процесс запущен.
...
Карточки дайджеста перед отправкой переводятся на простой русский через Groq. Английский заголовок в чат не показывается; если модель недоступна, остаётся короткая русская тема по рубрике. Чужой текст из RSS и Telegram не копируется: в карточке только тема и ссылка.

Внизу экрана reply-меню: «Темы», «Пост», «Я проверила», «В канал», «Бюджет», «Помощь». Кнопка «Бюджет» и команда `/costs` показывают, сколько токенов ушло на Groq (сейчас это перевод тем дайджеста). Черновик кнопки «Пост» модель не вызывает. Если задан `DATABASE_URL`, счётчик пишется в PostgreSQL и переживает перезапуск; иначе живёт в памяти процесса. У карточек дайджеста — кнопки «Развить», «Сохранить», «Не интересно», «Открыть источник», плюс синяя ссылка в тексте карточки. «Развить» даёт короткий черновик и обложку. После черновика — «Доработать», «Новый текст», «Я проверила», «В канал».

Картинку можно прислать в чат (или команда `/cover`): бот ничего сам не обрезает. Под фото — «Значок», «Верх», «Низ», «Текст». После «Значок» — стрелки (куда поставить) и «Мельче»/«Крупнее». Результат в канал сам не уходит.
