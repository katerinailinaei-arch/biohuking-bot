...
from bodrye_bot.digest.service import Digest, DigestCandidate, DigestCard, SourceFailure
...
async def test_weekday_worker_delivers_partial_digest_once_and_records_lateness() -> None:
    ...
    worker = DigestWorker(owner_id=42, loader=loader, runs=runs, telegram=telegram)
    now = datetime(2026, 9, 1, 7, 5, tzinfo=UTC)  # Tuesday, 10:05 Moscow
    ...
    assert "Некоторые источники сегодня недоступны" in telegram.calls[0][1]
